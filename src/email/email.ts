...
export const sendEmailToForgotPassword = async (email: string, username: string, token: string) =>{
	const htmlWithCssTemplate = await getTemplate('forgot-password');
	const dataToTemplate = [
		{var: '{USERNAME}', value: username},
		{var: '{TOKEN}', value: token},
		{var: '{API}', value: `${ENV_SETUP.API_SERFLIX}/forgot-password`},
	];
	const htmlWithDataEmbedded = replaceDataOnTemplate(htmlWithCssTemplate, dataToTemplate);
	const mailOptions = {
		from: ENV_SETUP.EMAIL,
...
// If you want to use a template with variables into it, you can use this function and get a new template with the variables replaced
